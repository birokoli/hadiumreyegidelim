...
export default function AdsAdminPage() {
  ...
  return <div className="mx-auto max-w-6xl p-5 md:p-8">
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <Section title="Hazır Program CTA" description="Ana sayfadaki Adım Adım Yolculuk bölümünden hemen sonra gösterilir."><div className="grid gap-5 md:grid-cols-2"><Field label="Üst Etiket" value={form.readyCtaKicker} onChange={(v) => setField("readyCtaKicker", v)} /><Field label="Başlık" value={form.readyCtaTitle} onChange={(v) => setField("readyCtaTitle", v)} /><Field label="Açıklama" rows={3} value={form.readyCtaDescription} onChange={(v) => setField("readyCtaDescription", v)} /><Field label="Alt Not" value={form.readyCtaNote} onChange={(v) => setField("readyCtaNote", v)} /><Field label="Buton" value={form.readyCtaButton} onChange={(v) => setField("readyCtaButton", v)} /></div></Section>

      <Section title="Footer Öncesi Son ADS" description="Ana sayfadaki SSS bölümünün altında, footer'dan hemen önce gösterilir."><div className="grid gap-5 md:grid-cols-2"><Field label="Rozet Metni" value={form.finalAdsBadge} onChange={(v) => setField("finalAdsBadge", v)} /><Field label="Başlık" value={form.finalAdsTitle} onChange={(v) => setField("finalAdsTitle", v)} /><Field label="Açıklama" rows={3} value={form.finalAdsDescription} onChange={(v) => setField("finalAdsDescription", v)} /><Field label="Alt Not" value={form.finalAdsNote} onChange={(v) => setField("finalAdsNote", v)} /><Field label="Buton" value={form.finalAdsButton} onChange={(v) => setField("finalAdsButton", v)} /></div></Section>
