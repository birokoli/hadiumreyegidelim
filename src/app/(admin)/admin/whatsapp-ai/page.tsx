...
type DashboardData = {
  ...
  connection: { gemini: boolean; whatsapp: boolean; model: string };
...
export default function WhatsAppAIPage() {
  ...
  const [tab, setTab] = useState<"overview" | "knowledge" | "test">("overview");
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-5 md:p-8">
    ...
    <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-200 bg-white p-2">
      {[["overview","Genel Bakış"],["knowledge","Asistan & Bilgi Tabanı"],["test","Yanıt Testi"]].map(([id,label]) => <button key={id} onClick={() => setTab(id as typeof tab)} className={`rounded-xl px-5 py-3 text-sm font-bold ${tab === id ? "bg-[#003781] text-white" : "text-slate-500 hover:bg-slate-50"}`}>{label}</button>)}
    </div>
    ...
    {tab === "overview" && <>
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        ...
        {data.conversations.length === 0 ? <div className="p-10 text-center text-sm text-slate-400">Henüz WhatsApp konuşması yok. Meta bağlantısı tamamlandığında mesajlar burada görünecek.</div> :
