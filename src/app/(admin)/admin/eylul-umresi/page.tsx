...
export default function AdsAdminPage() {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setAds(data.configs); setMessage({ ok: true, text: "Üç ADS sayfası kaydedildi ve canlı içerikler güncellendi." });
  ...
  return <div className="mx-auto max-w-6xl p-5 md:p-8">
    <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <Link href={activeAdTab === 1 ? "/eylul-umresi" : activeAdTab === 2 ? "/ilk-umrem" : "/hanim-umresi"} target="_blank" className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-[#003781] hover:bg-slate-50"><span className="material-symbols-outlined text-[18px]">open_in_new</span>Canlı Sayfayı Aç</Link>
    </div>
    ...
    <form onSubmit={save} className="space-y-6">
      ...
      <Section title="Çıkış Tarihleri ve Kontenjan"><div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3"><Field label="1. Çıkış Etiketi" value={form.departureOneLabel} onChange={(v) => setField("departureOneLabel", v)} /><Field label="1. Çıkış Tarihi" value={form.departureOne} onChange={(v) => setField("departureOne", v)} /><Field label="2. Çıkış Etiketi" value={form.departureTwoLabel} onChange={(v) => setField("departureTwoLabel", v)} /><Field label="2. Çıkış Tarihi" value={form.departureTwo} onChange={(v) => setField("departureTwo", v)} /><Field label="Toplam Kontenjan" value={form.capacity} onChange={(v) => setField("capacity", v)} /><Field label="Yer Ayır Butonu" value={form.reserveButton} onChange={(v) => setField("reserveButton", v)} /></div></Section>
